...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    # Startup
    logger.info(
        "Starting GuidelineTransform AI application", extra={"event": "startup"}
    )
    ...
    # Shutdown
    logger.info(
        "Shutting down GuidelineTransform AI application", extra={"event": "shutdown"}
    )
...
async def value_error_handler(request: Request, exc: ValueError) -> JSONResponse:
    ...
    return JSONResponse(
        status_code=400, content={"detail": str(exc), "error": "validation_error"}
    )
...
async def general_exception_handler(request: Request, exc: Exception) -> JSONResponse:
    ...
    logger.error(
        f"Unexpected error: {exc}", extra={"url": str(request.url)}, exc_info=True
    )
    return JSONResponse(
        status_code=500,
...
# Include API routes
app.include_router(routes_parse.router, prefix="/api/v1", tags=["Parse"])

app.include_router(routes_tasks.router, prefix="/api/v1", tags=["Tasks"])

app.include_router(routes_drafts.router, prefix="/api/v1", tags=["Drafts"])

app.include_router(routes_export.router, prefix="/api/v1", tags=["Export"])
